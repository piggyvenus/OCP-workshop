...
12.  Click `Overview` on left nav 
13. Click `Group Service` button in the “javaapp” —> select mongodb from the list
14. Click `OK`
15. Now they are linked together. Click `Application` in left nav → deployment → click onto javaapp deployment
...
MONGODB_PASSWORD : mongodb
MONGODB_DATABASE : mongodb
17. Click `Overview` on left nav and javaapp is alread redeploying
18. Click onto the route to test the application.
***add final pictures***
